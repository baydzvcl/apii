...
async function openBrowser(targetURL, browserProxy) {
  const promise = async (resolve, reject) => {
    ...
    try {
      colored(colors.COLOR_YELLOW, "Run Proxy: " + browserProxy);
...
async function startThread(targetURL, browserProxy, task, done, retries = 0) {
  if (retries === COOKIES_MAX_RETRIES) {
    ...
    try {
      ...
      const cookies = "pageTitle: " + response.title + " - proxyAddress: " + response.browserProxy + " - userAgent: " + response.userAgent + " - Cookies: " + response.cookies;
